...
vacant <- function(geno.maf,
  ...
  gt  <- if (length(geno.maf) == 1) {
    utf8ToInt(geno.maf) - utf8ToInt("0")
  } else {
    Reduce(`+`, lapply(geno.maf, function(s) utf8ToInt(s) - utf8ToInt("0")))
  }
  ...
  # ---- 3. Clustering ----
  clus <- cluster_score(score.vector, geno.maf, size.threshold)
