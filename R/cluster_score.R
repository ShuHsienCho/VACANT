...
#'
#' @param score.vals Numeric matrix or vector. Rows are variants, columns are score types.
#' @param geno.maf Character vector. Genotype strings for allele count weighting.
...
#' @export
cluster_score <- function(score.vals,
                          geno.maf,
                          size.threshold = 10,
  ...
  if (is.vector(score.vals)) {
    score.mat <- matrix(score.vals, ncol = 1)
    colnames(score.mat) <- "Score"
  } else {
    score.mat <- as.matrix(score.vals)
  }
  ...
  ac <- vapply(stringi::stri_extract_all_regex(geno.maf, "\\d"),
